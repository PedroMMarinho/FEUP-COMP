...
public class OllirTest {
    ...
    public void invokeStaticMethod() {
        var result = getOllirResult("extra/InvokeStatic.jmm");

        System.out.println("---------------------- OLLIR ----------------------");
        System.out.println(result.getOllirCode());
        System.out.println("---------------------- OLLIR ----------------------");

        // Get the OLLIR code as a single string
        var ollirCode = result.getOllirCode();

        String regex = "invokestatic\\s*\\(\\s*B\\s*,\\s*\"foo\"\\s*,\\s*1\\.i32\\s*\\)\\s*\\.V\\s*;";

        var pattern = Pattern.compile(regex);
        var matcher = pattern.matcher(ollirCode);

        assertTrue("Expected an invokestatic(B) call to method 'foo' with parameter 1.i32", matcher.find());
    }

    @Test
    public void invokeVirtualThis() {
        var result = getOllirResult("extra/MethodCallThis.jmm");
        ...
        String regex = "invokevirtual\\s*\\(\\s*this\\.A\\s*,\\s*\"foo\"\\s*,\\s*1\\.i32\\s*\\)\\s*\\.i32\\s*;";

        var pattern = Pattern.compile(regex);
        var matcher = pattern.matcher(ollirCode);

        assertTrue("Expected an invokevirtual(B) call to method 'foo' with parameter 1.i32", matcher.find());
    }

    @Test
    public void callToImportNoAssign() {
        var result = getOllirResult("extra/CallToImportNoAssign.jmm");
        ...
        String regex = "invokevirtual\\s*\\(\\s*b\\.B\\s*,\\s*\"foo\"\\s*,\\s*1\\.i32\\s*\\)\\s*\\.V\\s*;";
    ...
    @Test
    public void callToImportWithAssign() {
        var result = getOllirResult("extra/CallToImportWithAssign.jmm");
        ...
        String regex = "invokevirtual\\s*\\(\\s*b\\.B\\s*,\\s*\"foo\"\\s*,\\s*1\\.i32\\s*\\)";
